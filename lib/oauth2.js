...
function Strategy(options, verify) {
  ...
  var key = options.sessionKey || ('oauth2:' + url.parse(options.authorizationURL).hostname);
  options.store = new SessionStore({ key: key });
...
Strategy.prototype.authenticate = function(req, options) {
	...
	if (req.query && req.query.error_code && !req.query.error) {
		return this.error(new Error(req.query.error_message, parseInt(req.query.error_code, 10)));
	}
	...
	if(options.state === true) {
		OAuth2Strategy.prototype.authenticate.call(startegy, req, options);
	}
	else {
		this._stateStore.store(req, options.state, function(err) {
			if(err)
				return err;
			else {
				OAuth2Strategy.prototype.authenticate.call(startegy, req, options);
