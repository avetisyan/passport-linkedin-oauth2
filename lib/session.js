 ...
 * Store request state.
 *
 * This implementation simply saves a provided state
 *
 * @param {Object} req
 * @param {String} state
 ...
 * @api protected
 */
SessionStore.prototype.store = function(req, state, callback) {
...
SessionStore.prototype.verify = function(req, providedState, callback) {
	...
	var key = this._key;
	if (!req.session[key] && !req.session.ssoState) {
		return callback(null, false, { message: 'Unable to verify authorization request state.' });
	}

	var state = req.session[key]?req.session[key].state:req.session.ssoState;
	if (!state) {
		return callback(null, false, { message: 'Unable to verify authorization request state.' });
	}
